...
- purchase_request / quotation / sales_order / report:
...
- report:
    Governs the "Reports" nav dropdown and its three routes (All Sales,
    All Purchase, Profit & Loss). Same on/off rules as the modules above.

Dashboard KPI cards follow suit: a module's KPI card(s) only render when
BOTH the feature flag is on AND the user has view permission for it, so
toggling a flag off here also removes its numbers from the dashboard.
